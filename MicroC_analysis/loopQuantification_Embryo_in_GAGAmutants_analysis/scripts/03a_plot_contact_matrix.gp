...
set size 0.25,0.25
set tic scale 0
set title "WT (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_WT" u 1:2:3 notitle w image #pixel
set ytics format ""
unset ytics
...
set size 0.25,0.25
set tic scale 0
set title "(GA)n mut2 (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_GAnmut2" u 1:2:3 notitle w image #pixel
set colorbox

set origin 0.50,0
set size 0.25,0.25
set tic scale 0
set title "(GA)n mut4 (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) print i+0.5,j+0.5,m[i,j]}' _tmp_matrix_GAnmut4" u 1:2:3 notitle w image # pixel

set origin 0.75,0
set size 0.25,0.25
set tic scale 0
set title "(GA)n mut6 (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) print i+0.5,j+0.5,m[i,j]}' _tmp_matrix_GAnmut6" u 1:2:3 notitle w image # pixel
