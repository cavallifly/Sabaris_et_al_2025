...
centralRegion = args[[1]]

#distPEscan_minDist3_maxDist10000_Loops_Embryo_renamedLoops_ext_1000bp_Loops_Embryo_renamedLoops_ext_1000bp_min3_max10000_range4_N408_res250_peScan_centralRegion_8pixels_heatmaps_data.txt
inFiles = list.files("./",pattern=paste0(".*",centralRegion,"pixels_heatmaps_data.txt.*"))
print(inFiles)
#quit()
...
# Using Generic RGB
#colors=c(rep(c(rgb( 0,   0,    225, 1.0, names = NULL, maxColorValue = 255), rgb( 255, 128, 0, 1.0, names = NULL, maxColorValue = 255)),6))
#print(head(data))

#levels = c("All_Control", "All_PH_KD", "noPcG_Control", "noPcG_PH_KD", "PcG_Control", "PcG_PH_KD")
...
for(inFile in inFiles)
{
    ...
    data <- allData
    data$condition <- gsub("GAFoptimizedUpTo5All_Control","All",data$condition)
    data$condition <- gsub("GAFoptimizedUpTo5_Control","GAF",data$condition)
    data$condition <- gsub("GAFoptimizedUpTo5Recentered_Control","GAFRecentred",data$condition)
    data$condition <- gsub("GAFoptimizedUpTo5Cluster2All_Control","cl2All",data$condition)
    data$condition <- gsub("GAFoptimizedUpTo5Cluster2_Control","cl2GAF",data$condition)
    data$condition <- gsub("GAFoptimizedUpTo5Cluster2Recentered_Control","cl2GAFRecentred",data$condition)
    ...
    levels <- c("WT", "GAnmut2", "GAnmut4", "GAnmut6")
    custom_pairs <- list(c(levels[[1]],levels[[2]]),c(levels[[1]],levels[[3]]),c(levels[[1]],levels[[4]]),
                         c(levels[[2]],levels[[3]]),c(levels[[2]],levels[[4]]),
    		         c(levels[[3]],levels[[4]]))
    # List of specific comparisons (custom pairs)
    #custom_pairs <- list(c("GAFoptimizedUpTo5All_Control","GAFoptimizedUpTo5_Control"), c("GAFoptimizedUpTo5All_Control","GAFoptimizedUpTo5Recentered_Control"), c("GAFoptimizedUpTo5_Control","GAFoptimizedUpTo5Recentered_Control"), c("GAFoptimizedUpTo5Cluster2All_Control","GAFoptimizedUpTo5Cluster2_Control"), c("GAFoptimizedUpTo5Cluster2All_Control","GAFoptimizedUpTo5Cluster2Recentered_Control"), c("GAFoptimizedUpTo5Cluster2_Control","GAFoptimizedUpTo5Cluster2Recentered_Control"))
    print(custom_pairs)
    #quit()


    #levels <- c("GAFoptimizedUpTo5All", "GAFoptimizedUpTo5", "GAFoptimizedUpTo5Recentered", "GAFoptimizedUpTo5Cluster2All", "GAFoptimizedUpTo5Cluster2", "GAFoptimizedUpTo5Cluster2Recentered")    
    colors=c(rgb( 108,   108,    108, 20, names = NULL, maxColorValue = 255), rgb( 163,  138, 195, 20, names = NULL, maxColorValue = 255), rgb(253, 246, 126, 20, names = NULL, maxColorValue = 255), rgb(226,   126,   129, 20, names = NULL, maxColorValue = 255))
    print(levels)
    ...
    bxp <- ggviolin(data, x="condition", y="values", color="black", fill="condition", alpha=.50, scale = "width", trim = TRUE) + scale_fill_manual(values=colors) +
	   ...
	   theme_classic() +
   	   theme(axis.text.x = element_text(angle = 60, hjust=1), legend.position = "none", axis.text=element_text(size=16))
	...
	if(subsetting == "TRUE")
	{
		...
		ypos <- c(ypos,yposStart + as.integer(i %% 3) * delta)
