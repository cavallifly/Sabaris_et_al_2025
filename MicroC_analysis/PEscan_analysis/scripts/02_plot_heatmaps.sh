...
for filePH18 in $(ls -1 *range4*text.mtx);
do
    ...
    sed -e "s/XXXhighXXX/${high}/g" -e "s/XXXlowXXX/${low}/g" -e "s/XXXcbminXXX/${cbmin}/g"  -e "s/XXXcbmaxXXX/${cbmax}/g" -e "s/XXXxminXXX/${xmin}/g" -e "s/XXXxmaxXXX/${xmax}/g" -e "s/XXXyminXXX/${ymin}/g" -e "s/XXXymaxXXX/${ymax}/g" -e "s/XXXsizeXXX/${size}/g" -e "s/XXXNXXX/${Nloops}/g" ../scripts/02_plot_heatmaps.gp | gnuplot
    ...
    conda run -n DEseq2 Rscript ../scripts/02_violinPlots_within_domain_scores.R $(awk -v c=${cbmin} 'BEGIN{print c*1.05}')
