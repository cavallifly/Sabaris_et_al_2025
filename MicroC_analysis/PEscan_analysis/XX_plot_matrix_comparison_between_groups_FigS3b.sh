...
do
    ...
    rsync -avz ../distPEscan_mC_*_minDist${minDist}_maxDist${maxDist}/RData/*WD_GAF_allPeaks_Histones_k_3_k?_*peScan_text.mtx .

    bash ../scripts/plot_matrix.sh
    
    outFile=distPEscan_mC_${microc}_WD_GAF_allPeaks_Histones_k_3_min${minDist}_max${maxDist}_range10_res250_peScan_centralRegion_8pixels_heatmaps_data.txt    
    rm -fr ${outFile}
...
conda run -n DEseq2 Rscript ../scripts/violinPlots_within_clusters_scores_FigS3.R 0
#Rscript ../scripts/violinPlots_within_clusters_scores_FigS3.R 0
