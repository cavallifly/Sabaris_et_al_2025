...
for(resolutions in list(c(1.0e3,1.0e3),c(3.0e3,3.0e3)))
{
	...
	for(annotated in c("FALSE","TRUE"))
	{
		...
		outFile = paste0("scoreMapsk",k,"kexp",kexp,"_",chr,"_",start,"bp_",end,"bp_r",res,"bp_Fig4_annot_",annotated,".png")
		...
		for(set in names(scoreData))
		{
		    ...
		    data   <- gextract(track,exIntrv,iterator=exIntrv,colnames="score")
	    	    data[is.na(data)] <- 0
	    	    outMatrix <- paste0("scoreMapsk",k,"kexp",kexp,"_",chr,"_",start,"bp_",end,"bp_r",res,"bp_Fig4_",set,".tsv")
		...
		png(file=paste0("scoreMapsk",k,"kexp",kexp,"_",chr,"_",start,"bp_",end,"bp_r",res,"bp_Fig4_annot_",annotated,"_toModify.png"),width=nc*slotSize,height=nr*slotSize)
