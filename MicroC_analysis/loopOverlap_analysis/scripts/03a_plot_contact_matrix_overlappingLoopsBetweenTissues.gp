...
plot "< awk 'BEGIN{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_Embryo" u 1:2:3 notitle w image
set ytics format ""
unset ytics
...
set size 0.33,0.33
set tic scale 0
set title "Eye-discs (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_PH18" u 1:2:3 notitle w image

set origin 0.66,0
set size 0.33,0.33
set tic scale 0
set title "Wing-discs (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_WD" u 1:2:3 notitle w image
