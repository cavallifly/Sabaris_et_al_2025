...
for filePH18 in $(ls -1 *mC_Embryo_*range4*text.mtx);
do
    ...
    sed -e "s/XXXhighXXX/${high}/g" -e "s/XXXlowXXX/${low}/g" -e "s/XXXcbminXXX/${cbmin}/g"  -e "s/XXXcbmaxXXX/${cbmax}/g" -e "s/XXXxminXXX/${xmin}/g" -e "s/XXXxmaxXXX/${xmax}/g" -e "s/XXXyminXXX/${ymin}/g" -e "s/XXXymaxXXX/${ymax}/g" -e "s/XXXsizeXXX/${size}/g" -e "s/XXXNXXX/${Nloops}/g" ./scripts/03a_plot_contact_matrix_overlappingLoopsBetweenEmbryos.gp | gnuplot
    ...
    awk -v s=$size -v c=$centralRegion '{low=(s+1)/2-int(c/2); high=(s+1)/2+int(c/2); if((low<($1+1) && ($1+1)<=high) && (low<($2+1) && ($2+1)<=high)){print "LD",$3}}'  _tmp_matrix_LD >> _tmp.txt
