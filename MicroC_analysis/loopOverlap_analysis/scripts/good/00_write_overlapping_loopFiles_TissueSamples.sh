...
do
    ...
    paste <(cat categories_at_${res}bp.txt) <(sed -e "s/S1/ ED_PH18 /g" -e "s/S2/ WD_WT /g" -e "s/S3/ Embryo_WT /g" categories_at_${res}bp.txt | awk '{for(i=1;i<=NF;i++){h[$i]++; if(h[$i]==1){printf("%s-",$i)}}; printf("\n") ; for(i=1;i<=NF;i++){h[$i]=0}}') | awk '{print $1,substr($2,1,length($2)-1)}' | sort -k 4,4 > renamed_categories_at_${res}bp.txt
