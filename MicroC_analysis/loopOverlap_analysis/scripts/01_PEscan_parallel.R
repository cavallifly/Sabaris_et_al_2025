...
#peakFiles <- list.files(path = ".", pattern = c(".+4000.+BeS.+bedpe"))
peakFiles <- list.files(path = ".", pattern = c(".+BeS.+bedpe"))
#peakFiles <- list.files(path = ".", pattern = c(".+AllE.+AllE.+2000.+BeS-Batut2020-Levo2022-Pollex2024-Dosten2025_in_BeS.bedpe"))
#peakFiles <- list.files(path = ".", pattern = c(".+AllE.+AllE.+.bedpe"))

#peakFiles <- list.files(path = ".", pattern = c(".+AllE.+AllE.+BeS-Dolsten2025.+in_BeS.bedpe"))
#peakFiles <- list.files(path = ".", pattern = c(".+AllE.+AllE.+BeS.+notIn_Dolsten2025.bedpe"))
#peakFiles <- list.files(path = ".", pattern = c(".+AllE.+AllE.+Dolsten2025.+notIn_BeS.bedpe"))
...
#foreach(row=1:1) %dopar% {
	...
	print(peIterator)
	    ...
	    outFolder <- '/zdata/data/mdistefano/2022_06_08_Project_on_PREs_contacts/04_loopCalling_analysis/peScan_analysis_loopOverlaps/overlaps_analysis_Embryo/'
	    #outFolder <- '/zdata/data/mdistefano/2022_06_08_Project_on_PREs_contacts/04_loopCalling_analysis/peScan_analysis_loopOverlaps/overlaps_analysis_tissues/'
