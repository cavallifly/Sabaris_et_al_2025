...
scoreTracks <- scoreData
pdf(paste0("PRE_loop_quantifications_k",k,"_kexp",kexp,"_Fig1G_",pTarget,"_phKD.pdf"),width=14)
...
for(i in 1:nrow(vPoints))
{
	...
	for(set in names(scoreTracks))
	{
	    ...
	    outMatrix <- paste0("scoreMapsk",k,"kexp",kexp,"_r",res,"bp_Fig1G_",set,"_phKD.tsv")
