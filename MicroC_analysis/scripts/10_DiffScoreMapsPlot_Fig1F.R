...
diffData <- list(
	larvae_double_vs_LD_larvae_DWT_k250_kexp250 = "hic.diffMaps.hic_larvae_double_merge_dm6_BeS_vs_hic_larvae_DWT_merge_dm6_BeS_chr2L_score_k250_kexp250_1Mb"	
)
print(diffData)
...
for(annotated in c("FALSE","TRUE"))
{
	...
	diffCol <- colorRampPalette(c("darkblue","lightgrey","darkred"))(200)	
        ...
        {
	    png(file=paste0("diffMaps_",chr,"_",start,"_",end,"_r",res,"bp_",tag,"_Fig1F_annot_",annotated,"_",set,".png"),width=nc*slotSize,height=nr*slotSize)	
	    ...
	    data[is.na(data)] <- 0
	    outMatrix <- paste0("diffMaps_",chr,"_",start,"_",end,"_r",res,"bp_",tag,"_Fig1F_",set,".tsv")
	...
	for(set in names(trackData))
	    ...
	    png(file=paste0("diffMaps_",chr,"_",start,"_",end,"_r",res,"bp_",tag,"_Fig1F_annot_",annotated,"_",set,"_toModify.png"),width=nc*slotSize,height=nr*slotSize)	
	    layout(matrix(1:(nr*nc),ncol=nc,byrow=TRUE),respect=TRUE)
	    par(mai=rep(0.45,4))
	...
	pdf(paste0('diffMaps_colorbar_Fig1F.pdf'),width=8,height=8)
