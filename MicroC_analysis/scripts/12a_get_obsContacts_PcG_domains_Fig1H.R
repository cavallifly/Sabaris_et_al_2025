...
outfile <- paste0("obsContacts_within_domains_in_",refChrom,".tab")
...
if(!file.exists(outfile))
{
	...
	#print(paste0(chrom1," ",start1," ",end1," ",chrom2," ",start2," ",end2))
	    ...
	    result <- data.frame(sample,chrom1,start1,end1,domain,chrom2,start2,end2,domain,refContacts,obsContacts,log(obsContacts/refContacts)/log(2))
    ...
    colnames(results) <- c("sample","chrom1","start1","end1","domain1","chrom2","start2","end2","domain2","refContacts","obsContacts",paste0("Log2FC_Contacts_",refSample))
