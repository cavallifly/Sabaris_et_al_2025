...
for(comparison in c("PcG_vs_PcG","Active_vs_Active","Null_vs_Null"))
{

inFiles = list.files("./",pattern=paste0("score.*All.*",comparison,".*thresholdDist.*data.tsv.*"))
print(inFiles)
#quit()
...
for(inFile in inFiles)
{
    ...
    #threshold = as.integer(str_split(inFile,"_")[[1]][[8]])
    #print(threshold)
    ...
    #custom_pairs <- made in a loop from the input

    levels1 = c(unique(data[grep("_10000-",data$condition),]$condition),unique(data[-grep("_10000-",data$condition),]$condition))
    if(length(levels1) == 0)
    {
        levels1 = c(unique(data[grep("_0-",data$condition),]$condition),unique(data[-grep("_0-",data$condition),]$condition))
    }
    levels <- levels1
    if(comparison == "PcG_vs_PcG")
    {
        colors = rep(c(rgb( 0,   0,    225, 1.0, names = NULL, maxColorValue = 255), rgb( 83, 83, 83, 1.0, names = NULL, maxColorValue = 255)),as.integer(length(levels)/2))
    }
    if(comparison == "Active_vs_Active")
    {
        colors = rep(c(rgb( 255,   0,    0, 1.0, names = NULL, maxColorValue = 255), rgb( 83, 83, 83, 1.0, names = NULL, maxColorValue = 255)),as.integer(length(levels)/2))
    }
    if(comparison == "Null_vs_Null")
    {
        colors = rep(c(rgb( 0,   0,    0, 1.0, names = NULL, maxColorValue = 255), rgb( 83, 83, 83, 1.0, names = NULL, maxColorValue = 255)),as.integer(length(levels)/2))
    }        
    custom_pairs <- list()
    j = 1
	...
	if(i %% 2 == 0)
	{
        ...
        custom_pairs[[j]] <- c(levels[[i]],levels[[i+1]])
	j <- j + 1	
    }
    print(levels)
    print(colors)
    ...
    summ <- data %>%
           group_by(condition) %>%
	   summarize(n = n(), condition=condition, score = min(data$values)-2)
    summ <- unique(summ)
    ...
    if (nrow(data) < 1000)
    {
	   ...
	   labs(y="Avg. domain-domain scores", x = "") +	   
	...
	checkNormalityp <- checkNormality$p.value
	print(checkNormality, quote=F)	
    } else {
	checkNormalityp <- 0.05
    }

    if(checkNormalityp > 0.05)
    {
	...
	print(paste0("This conclusion is supported by the Shapiro-Wilk test. The p-value is not significant (p = ",checkNormalityp,"),"), quote=F)
	print(paste0("so we can assume normality."), quote=F)
    } else {
        print(paste0("The normality assumption is not supported by the Shapiro-Wilk test. The p-value is significant (p = ",checkNormalityp,"),"), quote=F)
    ...
    finalBxp <- bxp +
	   #stat_pvalue_manual(data=pairwiseTestsCondition[pairwiseTestsCondition$p.adj < 0.05,], label="p.adj.formatted", label.size = 4, bracket.size = 0.2, tip.length = 0, ) +
	   stat_pvalue_manual(data=pairwiseTestsCondition, label="p.adj.formatted", label.size = 4, bracket.size = 0.1, tip.length = 0, ) +
