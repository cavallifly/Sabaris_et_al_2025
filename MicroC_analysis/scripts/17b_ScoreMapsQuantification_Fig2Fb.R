...
pre1Up   = gintervals("chr2L",16560363,16560364)
viewpoints <- list(
	   pre1Up = gintervals("chr2L",16560363,16560364),
	   pre2   = gintervals("chr2L",16485929,16485930)
	   )
...
scoreTracks <- scoreData
pdf(paste0("PRE_loop_quantifications_k",k,"_kexp",kexp,"_Fig2Fb_",pTarget,".pdf"),width=14)
...
for(i in 1:nrow(vPoints))
{
	...
	for(set in names(scoreTracks))
	{
	    ...
	    outMatrix <- paste0("scoreMapsk",k,"kexp",kexp,"_r",res,"bp_Fig2Fb_",set,".tsv")
	    write.table(data[,c(1,2,3,4,5,6,7)], file = outMatrix, sep="\t", row.names=FALSE, quote=FALSE)
	}
	jitterPlot(pointData[[pointName]],pTarget=pTarget,main=pointName, ylim=c(-100,75),jitWidth=.75,cols=c("darkslateblue",rep("darkslategrey",length(names(scoreTracks))-1)))
	####
    }
}
dev.off()
