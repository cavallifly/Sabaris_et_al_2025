...
insTracks <- list(
	...
	larvae_double_300 = 'insulation.INS_hic_larvae_double_merge_dm6_BeS_w300kb_r2kb',
        LE_WT_100    = 'insulation.INS_hic_LE_WT_merge_dm6_YOVL_w100kb_r2kb',
        LE_WT_150    = 'insulation.INS_hic_LE_WT_merge_dm6_YOVL_w150kb_r2kb',
        LE_WT_200    = 'insulation.INS_hic_LE_WT_merge_dm6_YOVL_w200kb_r2kb',
        LE_WT_250    = 'insulation.INS_hic_LE_WT_merge_dm6_YOVL_w250kb_r2kb',
        LE_WT_300    = 'insulation.INS_hic_LE_WT_merge_dm6_YOVL_w300kb_r2kb'	
        )
refSet = "larvae_DWT"
...
for(trackName in names(insData))
{
    ...
    outProfile <- paste0("insulationProfiles_Fig1D_",trackName,"_revisions.tsv")
...
cols[[3]]     <- "#00FF00"
fillCols[[3]] <- rgb(0, 255, 0, max = 255, alpha = 50, names = "green50")
print(cols)
...
for(set in names(insData))
{
    ...
    {
        pdf(paste0("insulationProfiles_Fig1D_larvae_double_revisions.pdf"),width=19,height=7)    
    ...
    if(set == "LE_WT")
    {
        pdf(paste0("insulationProfiles_Fig1D_LE_WT_revisions.pdf"),width=19,height=7)

	### Plot with larvae_DWT ###
	#plot(insData[[1]]$mean, type="l", ylim=c(0,1),xlim=c(0,nrow(insData[[1]])+nrow(insData[[1]])*0.15),xlab="", ylab="INSULATION", xaxt="n", frame=F)
	#polygon(c(1:nrow(insData[[refSet]]), nrow(insData[[refSet]]):1), c(insData[[refSet]]$mean-insData[[refSet]]$stddev,rev(insData[[refSet]]$mean+insData[[refSet]]$stddev)), col = fillCols[which(names(insData) == refSet)], border=F)
	...
	### Plot without larvae_DWT ###
	plot(insData[[set]]$mean, type="l", col=cols[which(names(insData) == set)], ylim=c(0,1),xlim=c(0,nrow(insData[[set]])+nrow(insData[[set]])*0.15),xlab="", ylab="INSULATION", xaxt="n", frame=F)

	axis(1,c(insIntrv$start,"PRE1","TAD1",insIntrv$end),at=c(1,pre1Pos,boundary1Pos,nrow(insData[[set]])))	
        legend("topright",legend=names(insData[c(3)]), col=cols[c(3)], pch=19)
    }
