...
do
	...
	then
	    ...
	    java -mx40G -jar ./scripts/ChromHMM.jar BinarizeBam -b ${binning} -t ${signalDir} -p 0.0001 ./scripts/chrom_sizes.txt ./${bamDir}/ cellmarkfiletable.tsv ${binarizedBams} 
	...
	then
		...
		for file in $(ls -1 ${peakDir}/*${condition}*);
		do			
		    echo $file $chrom
