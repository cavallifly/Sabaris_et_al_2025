...
#peakFiles <- list.files(path = ".", pattern = c("cluster.+bed"))
#peakFiles <- list.files(path = ".", pattern = c(".+Q.+bed"))
#peakFiles <- list.files(path = ".", pattern = c("rando.+bed"))
#peakFiles <- list.files(path = ".", pattern = c(paste0(sample,".+bed")))
#if(sample == "Embryo")
#{
#    peakFiles <- list.files(path = ".", pattern = c(paste0("_E_",".+bed")))
#}
peakFiles <- list.files(path = ".", pattern = c(".+bed"))
...
for(peak1 in 1:length(peakFiles))
{
    ...
    {    
        ...
        ### Proximal 1A
        minDist   <-  10e3   # Lower limit of the band to consider
    	maxDist   <- 500e3   # Upper limit of the band to consider
    	#maxDist   <- 300e3   # Upper limit of the band to consider	
    	range     <-  10e3   # Region to consider centered around the peak
	...
	### Proximal 1B
        minDist   <-  10e3   # Lower limit of the band to consider
    	maxDist   <- 500e3   # Upper limit of the band to consider
    	range     <-  5e3   # Region to consider centered around the peak
	p1 <- append(p1,peakFiles[peak1])
	p2 <- append(p2,peakFiles[peak2])
	p3 <- append(p3,minDist)
    	p4 <- append(p4,maxDist)
    	...
    	### Distal
    	minDist   <- 500e3  # Lower limit of the band to consider
    	maxDist   <- 5e6    # Upper limit of the band to consider 
    	range     <- 25e3   # Region to consider centered around the peak
	p1 <- append(p1,peakFiles[peak1])
	p2 <- append(p2,peakFiles[peak2])
	p3 <- append(p3,minDist)
    	p4 <- append(p4,maxDist)		
...
#foreach(row=1:1) %dopar% {
    ...
    {
        setName1 <- gsub('../peaks/','',gsub('.bed','',peak1))
        setName2 <- gsub('../peaks/','',gsub('.bed','',peak2))
	...
	print(peIterator)
	    ...
	    #######################################################
	    outFolder <- '/zdata/data/mdistefano/2022_06_08_Project_on_PREs_contacts/peScan_analysis/'
	    #outFolder <- ''
