...
inFiles = list.files("./",pattern=".*allPeaks_Histones_k_3_.*_range10_res250_peScan_centralRegion_8pixels_heatmaps_data.txt.*")
print(inFiles)
#quit()
...
for(inFile in inFiles)
{
    ...
    bxp <- ggviolin(data, x="condition", y="values", color="black", fill="white", scale = "width", trim = TRUE) + #scale_fill_manual(values=colors) +
