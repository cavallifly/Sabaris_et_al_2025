...
inFiles = list.files("./",pattern=".*allPeaks_development_k_6_min.*_range10_res250_peScan_centralRegion_8pixels_heatmaps_data.txt.*")
print(inFiles)
#quit()
...
levels = c("cl1","cl2","cl3","cl4","cl5","cl6")
...
for(inFile in inFiles)
{
    ...
    bxp <- ggviolin(data, x="condition", y="values", color="black", fill="white", alpha=0.50, scale = "width", trim = TRUE) + #scale_fill_manual(values=colors) +
           geom_boxplot(position=position_dodge(1), width=0.1, color="black", outlier.shape = NA) +
           geom_point(position = position_jitter(width=0.2), size = 0.5, alpha=1.0) +	   	   
