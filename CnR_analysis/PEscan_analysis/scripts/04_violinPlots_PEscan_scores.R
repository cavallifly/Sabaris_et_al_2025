...
inFiles = list.files("./",pattern=".*distPEscan_mC.*_res250_peScan_centralRegion_8pixels_heatmaps_data.txt.*")
print(inFiles)
#quit()
...
# Using Generic RGB
#colors=c("All" = rgb(52, 0, 67, 1.0, names = NULL, maxColorValue = 255), "Q4" = rgb(69, 13, 83, 1.0, names = NULL, maxColorValue = 255), "Q3" = rgb(56, 99, 135, 1.0, names = NULL, maxColorValue = 255), "Q2" = rgb(68, 180, 116, 1.0, names = NULL, maxColorValue = 255), "Q1" = rgb(253, 231, 56, 1.0, names = NULL, maxColorValue = 255), "Random" = rgb(253, 98, 36, 1.0, names = NULL, maxColorValue = 255))
colors=c("#CF6263","#6FE8F9","#F9E86F")
#colors=c(rgb(52, 0, 67, 1.0, names = NULL, maxColorValue = 255), rgb(69, 13, 83, 1.0, names = NULL, maxColorValue = 255), rgb(56, 99, 135, 1.0, names = NULL, maxColorValue = 255), rgb(68, 180, 116, 1.0, names = NULL, maxColorValue = 255), rgb(253, 231, 56, 1.0, names = NULL, maxColorValue = 255), rgb(253, 98, 36, 1.0, names = NULL, maxColorValue = 255))
print(head(data))

#data$condition = factor(data$condition, levels=c("All","Q4","Q3","Q2","Q1","Random"))
...
for(inFile in inFiles)
{
    ...
    levels <- c("Embryo","ED","WD")

    print(levels)
    data$condition = factor(data$condition, levels=levels)
    print(data)
    if(length(unique(data$condition)) < 3)
    {
        next
    }
    #quit()
    ...
    summ <- data %>%
           group_by(condition) %>%
	   summarize(n = n(), condition=condition, score = ((1-0.20)*min(data$values)))
    summ <- unique(summ)
    print(as.data.frame(summ), quote=F)   
    #quit()

    bxp <- ggviolin(data, x="condition", y="values", color="black", fill="condition", alpha=0.5, scale = "width", trim = TRUE) + scale_fill_manual(values=colors) +
           geom_boxplot(position=position_dodge(1), width=0.1, color="black", outlier.shape = NA) +
           geom_point(position = position_jitter(width=0.2), size = 0.5, color="black", alpha=1) +	   	   
