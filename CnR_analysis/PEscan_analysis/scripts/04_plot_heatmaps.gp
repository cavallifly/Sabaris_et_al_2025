...
set ytics format ""
unset ytics
set title "Embryo (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_Embryo" u 1:2:3 notitle w image #pixel

set origin 0.34,0
set size 0.33,0.33
set tic scale 0
set title "ED (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<=XXXsizeXXX;i++)for(j=0;j<=XXXsizeXXX;j++) print i+0.5,j+0.5,m[i,j]}' _tmp_matrix_ED" u 1:2:3 notitle w image # pixel
set colorbox

set origin 0.68,0
set size 0.33,0.33
set tic scale 0
set title "WD (n=XXXNXXX)"
plot "< awk 'BEGIN{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) m[i,j]=0.0}{v=$3; m[int($1),int($2)]=v}END{for(i=0;i<XXXsizeXXX;i++)for(j=0;j<XXXsizeXXX;j++) print i,j,m[i,j]}' _tmp_matrix_WD" u 1:2:3 notitle w image #pixel
